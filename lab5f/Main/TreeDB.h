...
private:
   ...
   bool insert_bst(TreeNode* n, TreeNode* r);

   DBentry* find_bst(TreeNode* r,string name);

   int cActive(const TreeNode* r)const;

   TreeNode* searchnode(TreeNode* r, string name);

   TreeNode* maxleftn(TreeNode* n);

   void clean(TreeNode* r);

   void printall(const TreeNode* r)const;
